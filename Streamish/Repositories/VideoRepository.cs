            ...
            using (var conn = Connection)
            {
                ...
                SELECT v.Id AS VideoId, v.Title, v.Description, v.Url, 
                       ...
                       c.Id AS CommentId, c.Message, c.UserProfileId AS CommentUserProfileId,
                        ...
                        uc.Name AS Commentor, uc.Id AS CommentorId, uc.ImageUrl AS CommentorAvatar
                  FROM Video v 
                       JOIN UserProfile up ON v.UserProfileId = up.Id
                       LEFT JOIN Comment c ON c.VideoId = v.id
                        JOIN UserProfile uc ON c.UserPRofileId = uc.Id
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (DbUtils.IsNotDbNull(reader, "CommentId"))
                        {
                            existingVideo.Comments.Add(new Comment()
                            {
                                ...
                                UserProfile = new UserProfile()
                                {
                                    Id =  DbUtils.GetInt(reader, "CommentorId"),
                                    Name = DbUtils.GetString(reader, "Commentor"),
                                    ImageUrl = DbUtils.GetString(reader, "CommentorAvatar")
                                },
                                VideoId = videoId,
                                UserProfileId = DbUtils.GetInt(reader, "CommentUserProfileId")
                            });
